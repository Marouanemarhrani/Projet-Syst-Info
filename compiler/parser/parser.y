...
typedef struct {int a,c,p;} Sym; typedef struct {int op,a,b,c;} Ins;
enum {ADD=1,MUL,SOU,DIV,COP,AFC,JMP,JMF,INF,SUP,EQU,PRI,ADR,LDP,STP};
static Sym syms[512]; static char *names[512]; static int ns,na,nt=128,err,loop_start; static Ins code[8192]; static int nc; static const char *out="program";
...
static void writeout(){char a[512],b[512];snprintf(a,512,"%s.asm",out);snprintf(b,512,"%s.obj",out);FILE*x=fopen(a,"w"),*y=fopen(b,"w");const char*n[]={"?","ADD","MUL","SOU","DIV","COP","AFC","JMP","JMF","INF","SUP","EQU","PRI","ADR","LDP","STP"};for(int i=0;i<nc;i++){Ins z=code[i];fprintf(x,"%04d: %s %d %d %d\n",i,n[z.op],z.a,z.b,z.c);if(z.op==JMP||z.op==PRI)fprintf(y,"%d %d\n",z.op,z.a);else if(z.op==AFC||z.op==COP)fprintf(y,"%d %d %d\n",z.op,z.a,z.b);else if(z.op==JMF)fprintf(y,"%d %d %d\n",z.op,z.a,z.b);else fprintf(y,"%d %d %d %d\n",z.op,z.a,z.b,z.c);}fclose(x);fclose(y);}
%}
...
primary: NUMBER {if(nt>=256){bad("mémoire temporaire pleine");}int a=nt++;emit(AFC,a,$1,0);$$=a;}|IDENTIFIER {$$=addr($1);free($1);};
%%
